...
class AnHuiCme:
    def get_cme_project(self, page_max_id: int, page_size: int, *args, **kwargs) -> list:
        table_metadata_all = []
        table_metadata_header = ['项目编号', '项目名称', '主办单位', '项目负责人', '负责人电话', '举办期限起止日期', '举办地点', '授予学分', '教学对象', '拟招生人数', '备注']
    ...
    def _get_html(self, page_id: int, page_size: int, *args, **kwargs) -> str:
        ...
        url = f'https://tianjincme.haoyisheng.com/tianjin_project/projectGongbuList.do?d-1342871-p={page_id}&beian=&orderBy=subject&pici=&parentSubjectId=&pageSize={page_size}&zdanwei=&type=&xmanager=&gongbu=3&gongbuCode=&subjectId=&name=&scode=&year=2024'
    ...
    def _table_data(self, html, *args, **kwargs) -> list:
        ...
        for row in rows:
            ...
            cols = [ele.text.strip().replace('\xa0', '').replace('\r', '').replace('\n', '').replace('\t', '') for ele in cols]

            if len(cols) != 0:  # 判断列表是否为空值
                cols.pop(-1)  # 去除最后一列表格数据
...
class metadataToFile:
    ...
    def toExcel(self, tabledata):
        ...
        # 保存工作簿到文件
        file_name = '天津CME_%s.xlsx' % (time.strftime('%Y-%m-%d'))
        wb.save(file_name)
...
if __name__ == '__main__':
    ...
    # table_all = cme_data_obj.get_cme_project(page_size=800, page_max_id=1)
    ...
    # EXCEL转化为INSERT SQL
    to_excel_obj.to_inster_sql2(excel_file='天津CME_2024-09-19.xlsx', sql_file_name='天津CME.sql')
