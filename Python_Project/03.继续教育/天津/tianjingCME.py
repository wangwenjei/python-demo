...
class TianJingCme:
    ...
    def _get_html(self, page_id: int, page_size: int, *args, **kwargs) -> str:
        ...
        year = 2025
        url = (f"https://tianjincme.haoyisheng.com/tianjin_project/projectGongbuList.do?"
               f"d-1342871-p={page_id}&beian=&orderBy=subject&pici=&parentSubjectId=&pageSize={page_size}&"
               f"zdanwei=&type=&xmanager=&gongbu=3&gongbuCode=&subjectId=&name=&scode=&year={year}")
...
if __name__ == '__main__':
    """1.获取表格数据"""
    cme_data_obj = TianJingCme()
    table_all = cme_data_obj.get_cme_project(page_size=100, page_max_id=9)
    # table_all = cme_data_obj.get_cme_project(page_size=10, page_max_id=2)

    """2.导入Excel"""
    to_excel_obj = metadataToFile()
    excel_filename = '天津CME_%s.xlsx' % (time.strftime('%Y-%m-%d'))
    sql_filename = '天津CME_INSERT_%s.sql' % (time.strftime('%Y-%m-%d'))
