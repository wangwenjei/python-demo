...
class ChongQingCme:
    ...
    def _get_html(self, page_id: int, page_size: int, *args, **kwargs) -> str:
        ...
        url = (f"http://www.xm.cqwsjsw.com/chongqing_project/projectGongbuList.do?"
               f"orgId=&projectLevel=2&orgIds=1%2C2%2C52800000&year=2025"
               f"&scode=&parentSubjectId=&subjectId=&type=&beian=&orderBy=subject&"
               f"gongbu=3&pici=&gongbuCode=&name=&xmanager=&zdanwei=&pageSize={page_size}&d-1342871-p={page_id}")
...
if __name__ == '__main__':
    """1.获取表格数据"""
    cme_data_obj = ChongQingCme()
    # table_all = cme_data_obj.get_cme_project(page_size=50, page_max_id=24)
    table_all = cme_data_obj.get_cme_project(page_size=10, page_max_id=1)

    """2.导入Excel"""
    to_excel_obj = metadataToFile()
    excel_filename = '重庆CME_%s.xlsx' % (time.strftime('%Y-%m-%d'))
    sql_filename = '重庆CME_INSERT_%s.sql' % (time.strftime('%Y-%m-%d'))
