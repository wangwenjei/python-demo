...
class Cme:
    ...
    def _get_html(self, page_id: int, page_size: int, *args, **kwargs) -> str:
        ...
        year = 2025
        url = (f"https://hebeicme.haoyisheng.com/hebeicme/projectGongbuList.do?"
               f"d-1342871-p={page_id}&beian=&orderBy=subject&pici=&pageSize={page_size}"
               f"&parentSubjectId=&type=&zdanwei=&orgId=3&gongbuCode=&gongbu=3&projectLevel=2&"
               f"xmanager=&name=&subjectId=&scode=&year={year}")
...
if __name__ == '__main__':
    """1.获取表格数据"""
    cme_data_obj = Cme()
    table_all = cme_data_obj.get_cme_project(page_size=100, page_max_id=10)
    # table_all = cme_data_obj.get_cme_project(page_size=10, page_max_id=1)

    """2.导入Excel"""
    to_excel_obj = metadataToFile()
    excel_filename = '河北CME_%s.xlsx' % (time.strftime('%Y-%m-%d'))
    sql_filename = '河北CME_INSERT_%s.sql' % (time.strftime('%Y-%m-%d'))
